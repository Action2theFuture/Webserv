...
int SocketManager::createSocket(int port)
{
    ...
    if (sockfd < 0)
    {
        LogConfig::logError("socket() failed for port " + numberToString(port));
        exit(EXIT_FAILURE);
    }
    return sockfd;
}
...
void SocketManager::bindSocket(int sockfd, int port)
{
    ...
    {
        LogConfig::logError("bind() failed for port " + numberToString(port));
        close(sockfd);
        exit(EXIT_FAILURE);
    }
}
...
void SocketManager::startListening(int sockfd, int port)
{
    ...
    {
        LogConfig::logError("listen() failed for port " + numberToString(port));
        close(sockfd);
        exit(EXIT_FAILURE);
    }
}
